...
fake_headers = {
    ...
    'en-US,en;q=0.8',
    'User-Agent':
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.74 Safari/537.36 Edg/79.0.309.43',  # noqa
}
...
def url_save(url,
            ...
            if not force and (file_size == os.path.getsize(filepath)
                ...
                if not is_part:
                    ...
                    if skip_existing_file_size_check:
                        log.w(
                            'Skipping {} without checking size: file already exists'
                            .format(tr(os.path.basename(filepath))))
                    else:
    ...
    for i, url in enumerate(urls):
        ...
        if received < file_size and received < chunk_end:
            ...
            if timeout:
                response = urlopen_with_retry(request.Request(
                    url, headers=tmp_headers),
                                              timeout=timeout)
            else:
                response = urlopen_with_retry(
            ...
            with open(temp_filepath, open_mode) as output:
                while True:
                    ...
                    if not buffer:
                        ...
                        tmp_headers['Range'] = 'bytes=' + str(
                            received - chunk_start) + '-'
                        response = urlopen_with_retry(
                            request.Request(url, headers=tmp_headers))
                        continue
                    output.write(buffer)
...
class SimpleProgressBar:
    ...
    def update(self):
        ...
        if plus > 0.8:
            ...
            plus = ''
        bar = '█' * dots + plus
        bar = self.bar.format(percent, round(self.received / 1048576,
                                             1), bar, self.current_piece,
                              self.total_pieces, self.speed)
...
def print_user_agent(faker=False):
    ...
    user_agent = fake_headers[
        'User-Agent'] if faker else urllib_default_user_agent
...
def download_urls(urls,
    ...
    if total_size:
        if not force and os.path.exists(output_filepath) and not auto_rename\
            ...
            if skip_existing_file_size_check:
                log.w(
                    'Skipping %s without checking size: file already exists' %
                    output_filepath)
            else:
    ...
    if len(urls) == 1:
        ...
        bar.update()
        for i, url in enumerate(urls):
            output_filename_i = get_output_filename(urls,
                                                    title,
                                                    ext,
                                                    output_dir,
                                                    merge,
...
def download_rtmp_url(url,
    ...
    from .processor.rtmpdump import (has_rtmpdump_installed,
                                     download_rtmpdump_stream)
    assert has_rtmpdump_installed(), 'RTMPDump not installed.'
    download_rtmpdump_stream(url, title, ext, params, output_dir)


def download_url_ffmpeg(url,
                        title,
                        ext,
                        params={},
                        total_size=0,
...
def print_info(site_info, title, type, size, **kwargs):
    if json_output:
        json_output_.print_info(site_info=site_info,
                                title=title,
                                type=type,
                                size=size)
        return
...
def set_socks_proxy(proxy):
    try:
        ...
        if '@' in proxy:
            ...
            socks.set_default_proxy(socks.SOCKS5, socks_proxy_addrs[0],
                                    int(socks_proxy_addrs[1]), True,
                                    socks_proxy_auth[0], socks_proxy_auth[1])
        else:
...
def script_main(download, download_playlist, **kwargs):
    ...
    def print_version():
        version = get_version(kwargs['repo_path'] if 'repo_path' in
                              kwargs else __version__)
    ...
    download_grp.add_argument('-a',
                              '--auto-rename',
                              action='store_true',
                              default=False,
                              help='Auto rename same name different files')

    download_grp.add_argument('-k',
                              '--insecure',
                              action='store_true',
                              default=False,
    ...
    proxy_grp.add_argument('-s',
                           '--socks-proxy',
                           metavar='HOST:PORT or USERNAME:PASSWORD@HOST:PORT',
...
def google_search(url):
    ...
    for v in zip(videos, durs):
        print('- video:  {} [{}]'.format(unescape_html(v[0][1]),
                                         v[1] if v[1] else '?'))
...
def url_to_module(url):
    ...
    url = ''.join(
        [ch if ord(ch) in range(128) else parse.quote(ch) for ch in url])
    video_host = r1(r'https?://([^/]+)/', url)
    video_url = r1(r'https?://[^/]+(.*)', url)

    k = r1(r'([^.]+)', domain)
    if k in SITES:
        return (import_module('.'.join(['you_get', 'extractors',
                                        SITES[k]])), url)
    else:
        try:
